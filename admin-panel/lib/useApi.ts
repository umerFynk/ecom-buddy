...
export function useApi<T = unknown>(url: string, deps: unknown[] = []) {
  ...
  const fetcher = useCallback(async () => {
    ...
    try {
      ...
      setError((e as { response?: { data?: { error?: string } }; message?: string }).response?.data?.error ?? (e as { message?: string }).message ?? 'Request failed');
